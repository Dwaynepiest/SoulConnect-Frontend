    ...
    try {
        ...
        db.query('SELECT * FROM users WHERE email = ?', [email], async (err, results) => {
            ...
            if (results.length > 0) {
                ...
                const docent = { name, email, password: hashedPassword, role_id: 2 }; // Default role_id = 1
                db.query('INSERT INTO users SET ?', docent, (err, result) => {
                    if (err) throw err;

                    // Generate token with the user's role (role_id = 1)
                    const token = generateToken(result.insertId, 2); // Role ID 1 corresponds to 'user'
